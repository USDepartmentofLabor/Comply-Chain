import React from "react";
import ExampleInAction from "../../../../../../components/ExampleInAction/ExampleInAction";
import StaticData from "../../../../../../components/StaticData";
// import ilo from "../../../../../images/ilo.jpg";
import OwnWords from "../../../../../../components/OwnWords";

const ms = () => {
    return (
        <StaticData>
            <div>
                <h1>
                    Menangani Kelemahan Migran dalam Kod Tingkah Laku
                </h1>
                <p>
                    Sering, pekerja yang terdedah kepada buruh paksa/ pemerdagangan adalah buruh migran dan buruh sementara, yang menjadi sebahagian besar pekerja dalam
                    industri tertentu dan di sebahagian negara di dunia. Dalam beberapa hal, majikan mendapatkan buruh migran dan buruh sementara melalui broker buruh atau
                    perkhidmatan perekrut. Pekerja sering terdedah kepada pelbagai penderaan dalam situasi ini. Ada perekrut yang merampas dokumen pengenalan pekerja untuk
                    memerangkap mereka, dan/atau menipu pekerja tentang jenis pekerjaan yang mereka akan lakukan, keadaan tempat kerja, tempat tinggal dan urusan kehidupan,
                    serta aspek lain dalam pekerjaan mereka. Dalam beberapa kes, pekerja dipaksa membayar yuran kepada perekrut, broker dan pelaku lain dalam rantai pekerja
                    migran - yuran yang mungkin mustahil untuk dibayar balik dan akibatnya terikat dengan hutang. Seperti yang termaktub dalam Konvensyen ILO 181, semua kos
                    merekrut buruh patut dibayar oleh majikan—pekerja tidak patut langsung dibebani kos untuk mendapatkan pekerjaan.
                </p>
                <p>
                Sekiranya anda mengetahui atau mengesyaki ada buruh migran di dalam rantai bekalan anda, amat penting untuk memasukkan standard dan peruntukan berkenaan di dalam kod tingkah laku anda.
                </p>
                <OwnWords>
                    <h3>
                        <OwnWords.Icon alt="Feedback"/> <strong>Contoh dalam Tindakan: Standard Pekerja Migran Patagonia*</strong>
                        {": "}
                    </h3>
                    <p>
                        Apabila audit tahun 2011 menunjukkan terdapatnya buruh paksa, pemerdagangan manusia, dan eksploitasi buruh di dalam kilang pembekal Patagonia di Taiwan, syarikat pakaian
                        sukan itu sedar bahawa walaupun wujud Kod Tingkah Laku Tempat Kerja Pembekal, standard yang lebih kukuh diperlukan.  Patagonia bekerjasama dengan Verité*, sebuah NGO
                        yang mempromosikan keadaan selamat, adil, dan mengikuti undang-undang di seluruh dunia, untuk melakukan kajian bagi memahami dengan lebih baik situasi pekerja migran dalam
                        rantai bekalan mereka.  Apabila mereka mendapati ramai migran menjadi mangsa sistem merekrut yang mengeksploitasi pekerja yang dibebani pula dengan yuran broker yang melampau
                        dan terdedah kepada perangkap hutang, Patagonia membuat standard komprehensif bagi pekerja migran.  Standard ini melarang kilang pembekal daripada mengenakan yuran merekrut
                        pekerja migran, termasuk untuk hidup dan kerja, dan prosedur untuk membuat aduan.  Standard Penggajian Pekerja Migran Patagonia diumumkan kepada awam melalui semakan pada tahun 2020.
                        Patagonia menyatakan bahawa dengan kerjasama pembekal untuk melaksanakan standard ini dan dengan jenama lain turut serta dalam usaha ini, “Patagonia telah membuat kemajuan dalam
                        menggerakkan pembekal mereka kepada sistem di mana pekerja tidak lagi perlu membayar yuran untuk direkrut.”
                    </p>
                    <p>
                        Sumber:{" "}
                        <a
                            target="_blank"
                            rel="noopener noreferrer"
                            href="https://www.patagonia.com/stories/the-unacceptably-high-cost-of-labor-a-new-migrant-worker-standard-from-patagonia/story-17743.html"
                        >
                        https://www.patagonia.com/stories/the-unacceptably-high-cost-of-labor-a-new-migrant-worker-standard-from-patagonia/story-17743.html
                        </a>.
                    </p>
                    <p>
                    Sumber:{" "}
                    <a
                        target="_blank"
                        rel="noopener noreferrer"
                        href="https://www.patagonia.com/our-footprint/migrant-workers.html"
                    >
                    https://www.patagonia.com/our-footprint/migrant-workers.html
                    </a>.
                    </p>
                </OwnWords>

                <p>
                    Dokumen dasar penting yang memberi fokus kepada pekerja migran yang terancam ialah
                    <a
                        target="_blank"
                        rel="noopener noreferrer"
                        href="http://www.dhaka-principles.org/pdf/2012-12-18-Dhaka-Principles-Long-Version-English.pdf"
                        title="Dhaka Principles for Migration with Dignity"
                    >
                        {" "}
                        Prinsip Dhaka untuk Berpindah dengan Bermaruah
                    </a>{" "}
                    (Prinsip Dhaka). Prinsip Dhaka mengandungi sepuluh prinsip yang menjejak pekerja mulai dari pengambilan, penggajian,
                    sehingga ke akhir hubungannya dengan pekerjaannya, dan memberi prinsip utama yang majikan dan perekrut migran harus hormati bagi memastikan penghijrahan yang bermaruah. Prinsip Dhaka direka bentuk oleh{" "}
                    <a
                        target="_blank"
                        rel="noopener noreferrer"
                        href="https://www.ihrb.org/"
                        title="Institute for Human Rights and Business"
                    >
                        Institut Hak Asasi Manusia dan Perniagaan (IHRB)
                    </a>{" "}
                     dan disokong oleh perniagaan, kerajaan, kesatuan sekerja, dan masyarakat sivil. Ia berdasarkan standard pertubuhan hak asasi manusia dan tiga tonggak{" "}
                    <a
                        target="_blank"
                        rel="noopener noreferrer"
                        href="http://www.ohchr.org/Documents/Publications/GuidingPrinciplesBusinessHR_EN.pdf"
                        title="UN Guiding Principles on Business and Human Rights"
                    >
                        Prinsip Panduan PBB mengenai Perniagaan dan Hak Asasi Manusia
                    </a>
                    .
                </p>

                <ExampleInAction id="leadership">
                    <h2>
                        <ExampleInAction.Icon alt="Lightbulb"/>
                        <strong>
                            Contoh dalam Tindakan: Kumpulan Kepimpinan untuk Pengambilan Pekerja yang Bertanggungjawab
                        </strong>
                    </h2>
                    <p>
                        Dilancarkan pada Mei,{" "}
                        <a
                            target="_blank"
                            rel="noopener noreferrer"
                            href="https://www.ihrb.org/employerpays/leadership-group-for-responsible-recruitment"
                            title="Leadership Group for Responsible Recruitment"
                        >
                            Kumpulan Kepimpinan untuk Perekrutan Bertanggungjawab
                        </a>*{" "}
                        merupakan kerjasama di antara beberapa syarikat multinasional yang besar dan pertubuhan pakar-pakar untuk memecut perubahan positif
                        dalam cara pekerja migran direkrut.  Ahli Kumpulan Kepimpinan komited dalam Prinsip Majikan Membayar:  <em>“Tiada pekerja patut membayar untuk pekerjaan -
                        kos untuk pengambilan sepatutnya ditanggung tidak oleh pekerja tetapi oleh majikan.”</em>
                    </p>
                    <p>
                        Ditubuh oleh badan bukan kerajaan{" "}
                        <a
                            target="_blank"
                            rel="noopener noreferrer"
                            href="https://www.ihrb.org/"
                            title="Institute for Human Rights and Business"
                        >
                            Institut Hak Asasi Manusia dan Perniagaan (IHRB)
                        </a>*{" "}
                         (IHRB), Kumpulan Kepimpinan menggesa Prinsip Majikan Membayar diserapkan dalam dasar dan amalan korporat dan kerajaan bagi melindungi pekerja
                        migran dan melawan risiko buruh paksa dalam operasi mereka, perhubungan perniagaan, dan rantai bekalan global.   Bersama-sama, Kumpulan Kepimpinan
                        menyediakan platform untuk menyokong dan bekerjasama sebagai hab ilmu untuk berkongsi amalan, alat, dan panduan bagi pengambilan pekerja secara bertanggungjawab.
                    </p>
                    <p>
                        Ketahui lebih lanjut mengenai Kumpulan Kepimpinan dan alat mengenai cara merekrut beretika dengan melayari{" "}
                        <a
                            target="_blank"
                            rel="noopener noreferrer"
                            href="https://www.ihrb.org/employerpays/"
                            title="Responsible Recruitment Gateway"
                        >
                            Pintu Perekrutan Bertanggungjawab
                        </a>
                        {" "}di{" "}
                        <a
                            target="_blank"
                            rel="noopener noreferrer"
                            href="https://www.ihrb.org/employerpays/"
                            title="Responsible Recruitment Gateway"
                        >
                        https://www.ihrb.org/majikanbayar/
                        </a>.
                    </p>
                </ExampleInAction>
                <p>
                    Dalam beberapa hal, pekerja migran disediakan tempat tinggal oleh majikan mereka sebagai sebahagian daripada terma penggajian. Bagi majikan, ini bermakna merekrut dan
                    mengekalkan pekerja yang baik, dan untuk pekerja migran yang telah membuat perjalanan jauh dari tempat asal, urusan ini menyediakan tempat tinggal yang sangat diperlukan.
                    Bagaimanapun, tempat tinggal seperti itu mestilah selamat, bersih dan sesuai untuk pekerja. Pekerja, mestilah ada kebebasan untuk bergerak, dan hak untuk memilih tempat
                    duduk lain sekiranya mereka inginkan. Menurut ILO, sekiranya pekerja tinggal dalam “keadaan hidup yang terhina” dalam kawasan yang disediakan majikan, dan mereka dikenakan
                    hukuman atau ancaman yang menghalang mereka daripada meninggalkan kerja mereka, ini seperti mereka dalam keadaan buruh paksaan.
                </p>
            </div>
        </StaticData>
    );
};

export default ms;
