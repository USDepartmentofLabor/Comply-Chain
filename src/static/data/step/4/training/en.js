import React from "react";

const en = () => {
    return (
        <div>
            <ol>
                <li>
                    Cisco, Cody, Laura Gitman and Celine Suarez (Business for
                    Social Responsibility). &ldquo;Good Practices for Complying
                    with Licensors&rsquo; Social and Environmental Requirements:
                    A Practical Guide for Licensees.&rdquo; 2011; available from{" "}
                    <a href="https://www.bsr.org/our-insights/report-view/good-practices-for-complying-with-licensors-social-and-environmental-requir">
                        https://www.bsr.org/our-insights/report-view/good-practices-for-complying-with-licensors-social-and-environmental-requir
                    </a>
                    .
                </li>
                <li>
                    ILO and IOE.{" "}
                    <em>
                        How to do business with respect for children&rsquo;s
                        right to be free from child labour: ILO-IOE child labor
                        guidance tool for business.
                    </em>{" "}
                    2015; available from{" "}
                    <a
                        href="http://www.ilo.org/ipec/Informationresources/WCMS_IPEC_PUB_27555/lang--en/index.htm"
                        title="How to do business with respect for children&rsquo;s right to be free from child labour: ILO-IOE child labor guidance tool for business."
                    >
                        http://www.ilo.org/ipec/Informationresources/WCMS_IPEC_PUB_27555/lang--en/index.htm
                    </a>
                    .
                </li>
                <li>
                    ILO-SAPFL.{" "}
                    <em>
                        Combating Forced Labour: A Handbook for Employers and
                        Businesses 2<sup>nd</sup>edition
                    </em>
                    , 2015; available from{" "}
                    <a href="http://www.ilo.org/wcmsp5/groups/public/---ed_norm/---declaration/documents/publication/wcms_101171.pdf">
                        http://www.ilo.org/wcmsp5/groups/public/---ed_norm/---declaration/documents/publication/wcms_101171.pdf
                    </a>
                    .
                </li>
                <li>
                    International Trade Union Confederation
                    <em>. Mini Action Guide: Forced Labour. </em>2008; available
                    from{" "}
                    <a href="http://www.ituc-csi.org/IMG/pdf/guide_forced_labour_EN.pdf">
                        http://www.ituc-csi.org/IMG/pdf/guide_forced_labour_EN.pdf
                    </a>
                    .
                </li>
                <li>
                    Interfaith Center on Corporate Responsibility.{" "}
                    <em>
                        Best Practice Guidance on Ethical Recruitment of Migrant
                        Workers.
                    </em>{" "}
                    April 2017; available from{" "}
                    <a
                        href="http://www.iccr.org/sites/default/files/iccrsbestpracticeguidanceethicalrecruitment05.04.17.pdf"
                        title="Best Practice Guidance on Ethical Recruitment of Migrant Workers"
                    >
                        http://www.iccr.org/sites/default/files/iccrsbestpracticeguidanceethicalrecruitment05.04.17.pdf
                    </a>
                    .
                </li>
                <li>
                    IOM/OSCE/ILO.{" "}
                    <em>
                        Handbook on Establishing Effective Labour Migration
                        Policies in Countries of Origin and Destination.
                    </em>{" "}
                    2006; available from{" "}
                    <a
                        href="http://www.ilo.org/wcmsp5/groups/public/---ed_protect/---protrav/---migrant/documents/publication/wcms_203851.pdf"
                        title="Handbook on Establishing Effective Labour Migration Policies in Countries of Origin and Destination"
                    >
                        http://www.ilo.org/wcmsp5/groups/public/---ed_protect/---protrav/---migrant/documents/publication/wcms_203851.pdf
                    </a>
                    .
                </li>
                <li>
                    IOM/OSCE.{" "}
                    <em>
                        Training Modules on Labour Migration Management &ndash;
                        Trainer&rsquo;s Manual
                    </em>
                    . 2011; available from{" "}
                    <a href="http://www.iom.int/jahia/webdav/shared/shared/mainsite/published_docs/brochures_and_info_sheets/Training-Module.pdf">
                        http://www.iom.int/jahia/webdav/shared/shared/mainsite/published_docs/brochures_and_info_sheets/Training-Module.pdf
                    </a>
                    .
                </li>
                <li>
                    United Nations.{" "}
                    <em>Anti-Human Trafficking and Migrant Smuggling</em>.2010,
                    available from{" "}
                    <a href="http://www.unodc.org/unodc/en/frontpage/2010/September/un-launches-global-plan-of-action-against-human-trafficking.html">
                        http://www.unodc.org/unodc/en/frontpage/2010/September/un-launches-global-plan-of-action-against-human-trafficking.html
                    </a>
                    .
                </li>
                <li>
                    Verit&eacute;. <em>Fair Hiring Toolkit; </em>available from{" "}
                    <a href="http://helpwanted.verite.org/helpwanted/toolkit">
                        http://helpwanted.verite.org/helpwanted/toolkit
                    </a>
                    .
                </li>
                <li>
                    Verit&eacute;. <em>Responsible Sourcing Tool; </em>available
                    from{" "}
                    <a href="http://responsiblesourcingtool.org/">
                        http://responsiblesourcingtool.org/
                    </a>
                    .
                </li>
            </ol>
            <p>
                <em>
                    *Note: References to specific companies and non-governmental
                    entities within this guide are for informational purposes
                    only to demonstrate best practices that have been recognized
                    internationally, as of the app&rsquo;s release date.
                    Referencing the entities should not be interpreted as an
                    official endorsement of those entities, their products, or
                    services by the U.S. Department of Labor and their inclusion
                    should not be interpreted as a comprehensive review of these
                    entities&rsquo; practices in all areas.
                </em>
            </p>
        </div>
    );
};

export default en;
