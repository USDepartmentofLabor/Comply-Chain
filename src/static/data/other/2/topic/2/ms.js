import React from "react";
import StaticData from "../../../../../../components/StaticData";
import ExampleInAction from "../../../../../../components/ExampleInAction";
import betterwork from "../../../../../../static/images/betterwork.jpg";
import { Link } from "react-router-dom";

const en = () => {
    return (
        <StaticData>
            <div>
                <h1>Menyertai Program Pematuhan Sosial Yang Wujud</h1>
                <p>
                                    Di beberapa industri, kumpulan syarikat telah bergabung untuk mengembangkan
                                    sistem pematuhan sosial bersama atau dalam beberapa kes, komponen sistem - yang
                                    diterima pakai oleh setiap syarikat. Bergabung dengan rangkaian seperti ini biasanya
                                    memerlukan komitmen masa dan sumber untuk bekerjasama dengan syarikat lain, tetapi
                                    akhirnya boleh menjimatkan usaha dan sumber dengan memanfaatkan wawasan dan pengalaman
                                    syarikat lain. Adalah penting untuk belajar mengenai inisiatif yang wujud dalam industri
                                    anda sebelum memutuskan untuk mengikuti program yang wujud atau membangunkan program anda sendiri.
                                </p>
                                <ExampleInAction id="better-work-garment">
                                    <h4>
                                        <ExampleInAction.Icon alt="Lightbulb"/>
                                        <strong>Contoh dalam Tindakan:</strong>
                                        {": "}

                                        Program ‘Better Work’ (Kerja Lebih Baik) dalam Industri Pakaian{" "}
                                    </h4>
                                    <p>
                                        Program{" "}
                                        <a
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            href="http://betterwork.org/"
                                            title="Better Work"
                                        >
                                            Better Work (Kerja Lebih Baik)
                                        </a>,
                                        inisiatif bersama Pertubuhan Buruh Antarabangsa (ILO) dan Perbadanan Kewangan Antarabangsa
                                        (IFC), dibentuk untuk meningkatkan amalan dan produktiviti pekerja di sektor pakaian.
                                        Better Work  adalah program pemantauan kilang yang telus yang dilakukan oleh pemantau yang
                                        boleh dipercayai (ILO) di negara-negara yang mengambil bahagian.  Penasihat syarikat ILO yang
                                        direkrut  di peringkat tempatan mengunjungi kilang untuk menilai pematuhan standard asas buruh -
                                        mengenai kebebasan berkumpul dan tawar-menawar secara kolektif, diskriminasi  pekerjaan, buruh
                                        kanak-kanak dan buruh paksa - serta undang-undang negara mengenai pampasan,  hubungan kontraktor
                                        dan tempat kerja, keselamatan tempat pekerjaan dan kesihatan, dan waktu bekerja.,
                                        Better Work mengumumkan penilaian ini melalui{" "}
                                        <a
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            href="https://portal.betterwork.org/transparency"
                                            title="Transparency Portal"
                                        >
                                            Portal Ketelusan
                                        </a>
                                        , yang membolehkan pembeli antarabangsa membuat pesanan di kilang yang berusaha untuk meningkatkan
                                        pematuhan mereka. Program ini juga memberikan nasihat dan bantuan pemulihan kepada firma untuk
                                        meningkatkan pematuhan mereka. &nbsp;
                                        <em>Better Work</em> distruktur untuk menyelaraskan
                                        insentif pemilik kilang, pembeli, kerajaan dan pekerja dengan lebih
                                        berkesan untuk melindungi hak dan meningkatkan pendapatan.  Jabatan Buruh  AS
                                        adalah penyumbang utama program Better Work di beberapa buah negara bersama dengan kerajaan-kerajaan  lain serta institusi-institusi.
                                    </p>
                                    <img
                                        src={betterwork}
                                        alt="Work being done in a well-lit and clean factory"
                                    />
                                    <br />
                                    <div className="small-text">
                                        <p>
                                            &copy; Better Work Jordan/Photo by
                                            Ala&rsquo;a al Sukhni
                                        </p>
                                    </div>
                                </ExampleInAction>
                                <p>
                                    Kebanyakan persatuan industri mempunyai keperluan asas untuk mana-mana syarikat
                                    yang ingin menyertai. Contohnya, anggota{" "}
                                    <a
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        href="http://www.responsiblebusiness.org/"
                                        title="Responsible Business Alliance (RBA)"
                                    >
                                        Pakatan Perniagaan Bertanggungjawab
                                    </a>{" "}
                                    (RBA) iaitu (bekas Gabungan Warga Industri Elektronik - Electronics
                                    Industry Citizenship Coalition) komited kepada{" "}
                                    <a
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        href="http://www.eiccoalition.org/standards/"
                                        title="Code of Conduct"
                                    >
                                        Kod Tingkah Laku
                                    </a>{" "}
                                    bagi memastikan peningkatan berterusan dalam keadaan sosial, persekitaran dan
                                    tanggungjawab beretika dalam rantai bekalan mereka. Anggota RBA menunjukkan
                                    bahawa pematuhan kepada kod biasa ini melalui penilaian wajib, termasuk{" "}
                                    <a
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        href="http://www.eiccoalition.org/standards/vap/"
                                        title="Validated Assessment Process"
                                    >
                                        Proses Penilaian yang Disahkan
                                    </a>
                                    , yang menentukan standard untuk pengesahan pematuhan dan
                                    penilaian yang berkesan dan boleh dikongsi.
                                </p>
                                <p>
                                    Sama ada terdapat kumpulan dalam industri anda, anda mungkin ingin melihat
                                    sistem pematuhan sosial yang merangkumi beberapa industri. Satu daripada program tersebut ialah{" "}
                                    <a
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        href="http://www.ethicaltrade.org"
                                        title="Ethical Trading Initiative (ETI)"
                                    >
                                        Inisiatif Perdagangan Beretika (ETI)
                                    </a>
                                    . Melalui rundingan dengan kesatuan sekerja, badan bukan kerajaan
                                    (NGO) dan anggota korporat ETI membangunkan{" "}
                                    <a
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        href="http://www.ethicaltrade.org/eti-base-code"
                                        title="ETI Base Code"
                                    >
                                        Tata Asas ETI
                                    </a>
                                    . yarikat yang menyertai ETI mesti terima pakai Kod ini dan bersetuju {" "}
                                    <a
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        href="http://www.ethicaltrade.org/resources/principles-implementation"
                                        title="ETI&rsquo;s Principles of Implementation"
                                    >
                                        Prinsip Pelaksanaan ETI
                                    </a>
                                    . Syarikat akan menerima alat, latihan dan akses kepada rangkaian global ETI
                                    untuk mendapatkan sokongan menangani isu pematuhan setempat. Untuk maklumat lanjut, sila lihat{" "}
                                    <a
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        href="http://www.ethicaltrade.org/node/1192?lang=en"
                                        title="ETI's toolkit"
                                    >
                                        Peralatan ETI
                                    </a>
                                    <a
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        href="http://www.ethicaltrade.org/node/1650"
                                        title="resources for ethical trade"
                                    >
                                        {" "}
                                         sumber untuk perdagangan beretika
                                    </a>
                                    .
                                </p>
                                <p>
                                    <Link to="/steps/3">
                                        Langkah 1: Libatkan Pihak Berkepentingan dan Rakan Kongsi
                                    </Link>{" "}
                                    menawarkan banyak contoh kerjasama dan inisiatif yang anda boleh sertai.

                                </p>

            </div>
        </StaticData>
    );
};

export default en;
