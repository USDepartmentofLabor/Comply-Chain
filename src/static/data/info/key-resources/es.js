import React from "react";
import { Link } from "react-router-dom";
import BorderSeparator from "../../../../components/BorderSeparator";
import Icons from "../../../../components/Icons";
import StaticData from "../../../../components/StaticData";
import SweatAndToilLink from "../../../../components/SweatAndToilLink";
import { EiaList } from "./shared";

const es = () => {
    return (
        <StaticData>
            <div>
                <h2>M&aacute;s aplicaciones como esta:</h2>
                <p>Departamento de Estado de los Estados Unidos.</p>
                <p>
                    <em>
                        <i>Sweat &amp; Toil:</i>
                        <i> </i>
                        <i>
                            Child Labor, Forced Labor, and Human Trafficking
                            Around the World
                        </i>
                    </em>
                </p>
                <SweatAndToilLink />
                <br />
                <BorderSeparator />
                <h2>
                    <strong>Instrumentos clave:</strong>
                </h2>
                <p>
                    Oficina del Alto Comisionado de las Naciones Unidas para los
                    Derechos Humanos
                </p>
                <p>
                    <em>
                        Principios rectores de las Naciones Unidas sobre
                        empresas y derechos humanos
                    </em>
                </p>
                <p>
                    <a
                        target="_blank"
                        rel="noopener noreferrer"
                        href="http://www.ohchr.org/Documents/Publications/GuidingPrinciplesBusinessHR_EN.pdf"
                        title="Principios Rectores de las Naciones Unidas sobre empresas y derechos humanos"
                    >
                        http://www.ohchr.org/Documents/Publications/GuidingPrinciplesBusinessHR_SP.pdf
                    </a>
                </p>
                <BorderSeparator />
                <p>
                    Organizaci&oacute;n de Cooperaci&oacute;n y Desarrollo
                    Econ&oacute;micos
                </p>
                <p>
                    <em>
                        L&iacute;neas directrices para empresas internacionales
                    </em>
                </p>
                <p>
                    <a
                        target="_blank"
                        rel="noopener noreferrer"
                        href="http://www.oecd.org/corporate/mne/"
                        title="L&iacute;neas directrices para empresas internacionales"
                    >
                        https://www.oecd.org/daf/inv/mne/MNEguidelinesESPANOL.pdf
                    </a>
                </p>
                <BorderSeparator />
                <p>Organizaci&oacute;n Internacional del Trabajo</p>
                <p>
                    <em>
                        Servicio de Asistencia de la OIT para empresas sobre las
                        normas internacionales del trabajo
                    </em>
                </p>
                <p>
                    <a
                        target="_blank"
                        rel="noopener noreferrer"
                        href="http://www.ilo.org/empent/areas/business-helpdesk/lang--en/index.htm"
                        title="Servicio de Asistencia de la OIT para empresas sobre las normas internacionales del trabajo"
                    >
                        http://www.ilo.org/empent/areas/business-helpdesk/lang--es/index.htm
                    </a>
                </p>
                <BorderSeparator />
                <p>Organizaci&oacute;n Internacional del Trabajo</p>
                <p>
                    <em>
                        Lucha contra el trabajo forzoso:<i> </i>Manual para
                        empleadores y empresas
                    </em>
                </p>
                <p>
                    <a
                        target="_blank"
                        rel="noopener noreferrer"
                        href="http://www.ilo.org/global/topics/forced-labour/publications/WCMS_101171/lang--en/index.htm"
                        title="&gt;Lucha contra el trabajo forzoso:<i> </i>Manual para empleadores y empresas"
                    >
                        http://www.ilo.org/global/topics/forced-labour/publications/WCMS_105883/lang--es/index.htm
                    </a>
                </p>
                <BorderSeparator />
                <p>Organizaci&oacute;n Internacional del Trabajo</p>
                <p>
                    <em>
                        <i>
                            Checkpoints for Companies &ndash; Eliminating and
                            Preventing Forced Labour{" "}
                        </i>
                    </em>
                </p>
                <p>
                    <a
                        target="_blank"
                        rel="noopener noreferrer"
                        href="http://www.ilo.org/global/publications/WCMS_456961/lang--en/index.htm"
                        title="<i>Checkpoints for Companies &ndash; Eliminating and Preventing Forced Labour</i>"
                    >
                        http://www.ilo.org/global/publications/WCMS_456961/lang--en/index.htm
                    </a>
                </p>
                <BorderSeparator />
                <p>Organizaci&oacute;n Internacional del Trabajo</p>
                <p>
                    <em>
                        Herramienta de orientaci&oacute;n de la OIT y la OIE
                        sobre el trabajo infantil destinada a las empresas
                    </em>
                </p>
                <p>
                    <a
                        target="_blank"
                        rel="noopener noreferrer"
                        href="http://www.ilo.org/ipec/Informationresources/WCMS_IPEC_PUB_27555/lang--en/index.htm"
                        title="Herramienta de orientaci&oacute;n de la OIT y la OIE sobre el trabajo infantil destinada a las empresas"
                    >
                        http://www.ilo.org/ipec/Informationresources/WCMS_IPEC_PUB_27555/lang--en/index.htm
                    </a>
                </p>
                <BorderSeparator />
                <p>Foro de Bienes de Consumo</p>
                <p>
                    <em>
                        <i>
                            Business Actions Against Forced Labour &ndash;
                            Success Stories
                        </i>
                    </em>
                </p>
                <p>
                    <a
                        target="_blank"
                        rel="noopener noreferrer"
                        href="http://www.theconsumergoodsforum.com/strategic-focus/social-sustainability/forced-labour-case-studies"
                        title="<i>Business Actions Against Forced Labour &ndash; Success Stories</i>"
                    >
                        http://www.theconsumergoodsforum.com/strategic-focus/social-sustainability/forced-labour-case-studies
                    </a>
                </p>
                <BorderSeparator />
                <p>Verit&eacute;</p>
                <p>
                    <em>
                        <i>Responsible Sourcing Tool</i>
                    </em>
                </p>
                <p>
                    <a
                        target="_blank"
                        rel="noopener noreferrer"
                        href="http://www.responsiblesourcingtool.org/"
                        title="<i>Responsible Sourcing Tool</i>"
                    >
                        http://www.responsiblesourcingtool.org/
                    </a>
                </p>
                <BorderSeparator />
                <p>Verit&eacute;</p>
                <p>
                    <em>
                        <i>
                            Anti-Human Trafficking Business Authentication
                            Criteria
                        </i>
                    </em>
                </p>
                <p>
                    <a
                        target="_blank"
                        rel="noopener noreferrer"
                        href="http://responsiblesourcingtool.org/download/reports/Verite_BusinessAuthenticationCriteria_r1.pdf"
                        title="<i>Anti-Human Trafficking Business Authentication Criteria</i>"
                    >
                        http://responsiblesourcingtool.org/download/reports/Verite_BusinessAuthenticationCriteria_r1.pdf
                    </a>
                </p>
                <BorderSeparator />
                <h2>
                    <strong>Ejemplos en acci&oacute;n:</strong>
                </h2>
                <EiaList>
                    <li>
                        <Link to="/basics#better-work-garment">
                            Programa Better Work en la industria de la
                            vestimenta
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/basics#eicc">
                            C&oacute;digo de conducta de la Coalici&oacute;n
                            Ciudadana de la Industria Electr&oacute;nica (EICC).
                            Sistemas de gesti&oacute;n
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link
                            to="/steps/1/topic/3#engagement_gov_brazil"
                            id="engagement_gov_brazil"
                        >
                            Participaci&oacute;n del gobierno: La &ldquo;lista
                            sucia&rdquo; de Brasil y el Instituto del Pacto
                            Nacional para la Erradicaci&oacute;n del Trabajo
                            Esclavo
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/1/topic/4#rsn" id="rsn">
                            Red de abastecimiento responsable
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link
                            to="/steps/1/topic/4#res_min_trade"
                            id="res_min_trade"
                        >
                            Alianza p&uacute;blico-privada para el comercio
                            responsable de minerales
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/1/topic/4#sug_phil" id="sug_phil">
                            Esfuerzos de m&uacute;ltiples interesados para
                            erradicar el trabajo infantil en el sector de la
                            ca&ntilde;a de az&uacute;car en las Filipinas
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/1/topic/5#eclt" id="eclt">
                            Fundaci&oacute;n para la erradicaci&oacute;n del
                            trabajo infantil en el cultivo del tabaco (ECLT)
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/1/topic/5#clt" id="clt">
                            Plataforma sobre el trabajo infantil
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/1/topic/5#gbn" id="gbn">
                            Red mundial de empresas sobre trabajo forzoso y
                            trata de seres humanos de la OIT
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/1/topic/6#goodweave" id="goodweave">
                            GoodWeave
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/2/topic/5#med_rep" id="med_rep">
                            La importancia de los medios de comunicaci&oacute;n
                            al informar
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/2/topic/5#fish" id="fish">
                            Pescados y mariscos
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/2/topic/6#leber" id="leber">
                            Leber Jeweler: Proyecto de Jeweler para Asistir a
                            Birmania
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/2/topic/6#Isidro" id="Isidro">
                            Isidro Le&oacute;n-York
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/2/topic/6#lco" id="lco">
                            League Collegiate Outfitters: Oportunidades para los
                            j&oacute;venes desfavorecidos en El Salvador
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/2/topic/6#gap" id="gap">
                            Programa P.A.C.E. de Gap Inc.
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link
                            to="/steps/2/topic/7#Conflict_Free"
                            id="Conflict_Free"
                        >
                            La iniciativa Conflict-Free Sourcing{" "}
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/2/topic/7#mica" id="mica">
                            L&rsquo;Or&eacute;al y el abastecimiento responsable
                            de mica
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/3/topic/3#rst1" id="rst1">
                            Instrumento de abastecimiento responsable 1, ejemplo
                            de disposiciones del c&oacute;digo de conducta
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/3/topic/4#patagonia" id="patagonia">
                            Normas sobre trabajadores migrantes de Patagonia
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/3/topic/4#leadership" id="leadership">
                            El Leadership Group for Responsible Recruitment
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/3/topic/5#fair_labor" id="fair_labor">
                            Proceso de modificaci&oacute;n del c&oacute;digo de
                            la Asociaci&oacute;n Trabajo Justo (Fair Labor
                            Association o FLA)
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/4/topic/4#cocoa" id="cocoa">
                            La industria internacional del chocolate y el cacao:
                            la estrategia CocoaAction de la Fundaci&oacute;n
                            Mundial del Cacao
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link
                            to="/steps/5/topic/2#better_work_assessment"
                            id="better_work_assessment"
                        >
                            Equipos de evaluaci&oacute;n del programa Better
                            Work
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/5/topic/3#rst1a" id="rst1a">
                            Instrumento de abastecimiento responsable 1A, puntos
                            de referencia de buenas pr&aacute;cticas en la
                            contrataci&oacute;n y el empleo
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/5/topic/4#prereg" id="prereg">
                            Proceso previo a la inscripci&oacute;n
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/5/topic/8#cambodia" id="cambodia">
                            Vigilancia comunitaria del trabajo infantil en
                            Camboya
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/6/topic/1#turkey" id="turkey">
                            Colaboraci&oacute;n para abordar la
                            problem&aacute;tica del trabajo infantil migrante y
                            de temporada en la cosecha de avellanas en
                            Turqu&iacute;a
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link
                            to="/steps/6/topic/2#better_work"
                            id="better_work"
                        >
                            El proceso de remediaci&oacute;n del programa Better
                            Work: Comit&eacute;s Consultivos de Mejora del
                            Desempe&ntilde;o (CCMD)
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/7/topic/1#fla" id="fla">
                            El programa de acreditaci&oacute;n de la FLA para
                            encargados externos de la vigilancia independiente
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/7/topic/1#bonsucro" id="bonsucro">
                            El modelo de acreditaci&oacute;n de Bonsucro
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/7/topic/1#sai_model" id="sai_model">
                            Modelo internacional de rendici&oacute;n de cuentas
                            social
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/7/topic/3#bwi_eval" id="bwi_eval">
                            Evaluaci&oacute;n del impacto del programa Better
                            Work
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                    <BorderSeparator />
                    <li>
                        <Link to="/steps/8/topic/3#nike" id="nike">
                            Nike: Notificaci&oacute;n de las medidas de
                            remediaci&oacute;n
                            <Icons.ChevronRight />
                        </Link>
                    </li>
                </EiaList>
            </div>
        </StaticData>
    );
};

export default es;
